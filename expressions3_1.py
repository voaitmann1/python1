...
def CalcExpr_ConsiderOrder_NoBrackets(DataParts, trStrGiven):
    ...
    if Ngiven==1:
        ...
        if what_Alg1Query2==2:
            ...
            y0=prePart.getVal()
        #
        op01=prePart.GetOperatorAsN()#if N==1 so op01==1=="+"
    #
    ...
    while contin==1:
        ...
        if WasRecursio==0:
            ...
            print("there was  recursion in ef'l cycle step. Expr an ef'y calc'd recursio data: "+curPart.ToString())
        #
        if what_Alg1Query2==2:
            y1=curPart.getVal()
        elif what_Alg1Query2==1:
        ...
        print("..."+str(y0)+" "+operatorNToString(op01, what_Alg1Query2)+" "+str(y1)+"..."+ operatorNToString(op12, what_Alg1Query2)+" ...")
        #
        if N==L:
            contin=0
            y2=Calc(y0, op01, y1)
            y0=y2
            op01=op12
            rStr.dataPart.setVal(y0)
            rStr.dataPart.setOperatorN(op01)
            rStr.N=N
            print("expr to _continue:"+rStr.dataPart.ToString())
        elif trStrGiven.dataPart.QShutBrackets>0:
            contin=0
            N+=1
            rStr.N=N
            print("expr to continue_:"+rStr.dataPart.ToString())
            #rest Part o'rPart s'idy
        else:
            print ("Analyzing")
            if(op01Order>=op12Order):
                WasRecursio=0
                print("order of cur oper "+OperatorNToString(op12)+" is NOT higher  than of prev oper "+OperatorNToString(op01))
                if what_Alg1Query2==2:
                    if op01==1:
                        y2=y0+y1
                        if y2>1:
                    ...
                    op01=op12
                #
                rStr.dataPart.setVal(y0)#mab exnot
                rStr.dataPart.setOperatorN(op01)#mab exnot
                print("expr to continue:"+rStr.dataPart.ToString())
                if N<L:
                    N+=1
                    print("Now N="+str(N))
                else:
                    print("N="+str(N)+"=L="+str(L)+" - and last expr reached")
                #
                rStr.N=N
            elif op12Order>op01Order:
                if N<L:
                    print("order of cur oper "+OperatorNToString(op12)+" IS higher than of prev oper "+OperatorNToString(op01))  
                    N+=1
                    #nextPart=DataParts[N-1]#N new
                    #trStr.dataPart.nextPart
                    trStr.N=N
                    trStr.dataPart.setVal(y1)
                    trStr.dataPart.setOperatorN(op12)
                    #
                    print("need to start recursion: in: N="+str(N)+", expr to transmit:"+trStr.dataPart.ToString()+" :")
                    #
                    y1=trStr.dataPart.getVal()
                    op12=trStr.dataPart.GetOperatorAsN()
                    #
                    print("starting recursion: in: N="+str(N)+", expr to transmit:"+trStr.dataPart.ToString()+" :")
                    #
                    ...
                    op12Order=OperOrder(op12)
                    # 
                    print("returning(2) to fn Ngiven="+str(Ngiven)+", now N="+str(N))
                    #
                    print("..."+str(y0)+" "+operatorNToString(op01, what_Alg1Query2)+" "+str(y1)+"..."+ operatorNToString(op12, what_Alg1Query2)+" ...")
                    #
                    WasRecursio=1
                    #
                    # et nu go to cycleAnfang. Ma woab S ved, ze anot l'expr y1 lir ab dataFlow?
                    #
                else:
                    print("")
                #
            #
            #else:#op12Order>op01Order
