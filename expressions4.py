    ...
    while contin==1:
        ...
        if trStrGiven.dataPart.QShutBrackets>0:
            ...
            if op01Order>=op12Order or N==L or curPart.QShutBrackets>0: #if N>L, L=0 - contin=0 et no tic cycle
                #WasRecursio=0
                print("order of cur oper "+OperatorNToString(op12)+" is NOT higher than of PREV oper "+OperatorNToString(op01)+" => returning")
            ...
            else:#N<L => N+1<=L and op01Order<op12Order
                ...
                if op12Order>=op23order:
                    ...
                    WasRecursio=1
                    print("returned from recursion to Ngiven="+str(Ngiven)+", from N="+str(trStr.N)+"="+str(N)+" now next part: "+trStr.dataPart.ToString())
                #
                #if N<L:
                #    N=N+1
                #    print("N=N+1="+str(N)+", going to next cycle step")
                if N<L:
                    ...
                    print("N="+str(N)+"L="+str(L)+" -last expr reached now")
                    contin=0
                    rStr.N=N
