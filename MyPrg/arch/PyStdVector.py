...
class My1DArray:
    ...
    def Set(self, data):#ne test'd #2
        if isinstance(obj,My1DArray):
            self.row=copy.deepcopy(obj.row)
        elif isinstance(obj,list):
            self.row=copy.deepcopy(obj)
        else:
            element=copy.deepcopy(data)
            self.row.append(element)
    #
    def SetCells(self, data):#ne tested, ma gut #3
        cell=MyCell()
    ...
    #
    def GetSubRow(self, N1, N2, ReturnAsMy1DArray1List0=1): #20
        #R=[]
        #L=self.GetLength()
        ...
        RMA1D=[]
        L=self.GetLength()
        if N1>=1 and N2>=1 and N1<=N2 and N2<=L:
            Lst=self.row[N1:N2+1]
            RMA1D=My1DArray(Lst)
        if ReturnAsMy1DArray1List0==0:
            Y=Lst
        else:
            Y=RMA1D
        return Y
