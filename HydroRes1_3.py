...
#def __init__(self, k=0, curN=0, Conn_Suc0Par1=0, upper=0, Ress=[], NinUpper=0, vsh=0):
    ...
    def ToStringLong(self, vsh=0):
        ...
        R=R+" Coords: ["+str(self.vis_x(0))+"; "+str(self.vis_y(0))+"] "
        R=R+" SubElts"
        if self.Conn_Suc0Par1==0:
            R=R+" [--]"
        else:
            ...
            if (self.Conn_Suc0Par1 == 0):
                ...
                for  i in range(1, QElements - 1+1):
                    ...
                    if(vsh!=0):
                        ...
                        print( str(i) + ") Lcur=" + str(Lcur) + "+1 connector. now L=" + str(L))
                #}
                Lcur = self.elems[self.QElements - 1].vis_L(vsh)
                L = L + Lcur
                if(vsh!=0):
                    print( "( returning to R" + str(self.curN) + ")")
                    print( str(self.QElements) + ") Lcur=" + str(Lcur) + " L=" + str(L))
    ...
    def vis_yUpper(self, vsh):
        ...
        if (isinstance(self.elems, list) and len(self.elems)>0):
            ...
            if (self.Conn_Suc0Par1 == 0):
                ...
                yUpperMax = 0
                #for (int i = 1; i <= self.QElements; i++)
                for i in range(1, self.QElements+1):
    ...
    #{
        ...
        #{
            ...
            x = xUpper
            if(vsh!=0):
                print( "Part of: " + self.upper.ToString() + " xUpper=" + str(xUpper) + " = x =" + str(x))
            if (self.upper.Conn_Suc0Par1 == 0):
            #{
                x = x + 1
                if(vsh!=0):
    ...
    #{
        s = self.CoordsToStr(0);
        print( s)
        QElements=0
        if isinstance(self.elems, list) and len(self.elems)>0:
            #for (int i = 1; i <= self.QElements; i++)
            for i in range(1, QElements):
        #{
    ...
    #{
        if(vsh!=0):
            print( "Recursive search fn SeekElementByNPrimitive starts working, for: " + self.ToString())
        cur = self
        QElements=0
        ...
        #{
            R = cur
            if(vsh!=0):
                print( "Found at: " + R.ToString())
        #}
        else:
        #{
            if(vsh!=0):
                print( "Not Found at: " + self.ToString())
        #}
        #contin = !found;
        if found==0:
            contin=1
    ...
    #{
        ...
        #{
            ...
            if (self.QElements == 0 or ((x == xL or x == xR) and (y <= yL and y >= yU)) or ((y == yL or y == yU) and (x <= xR and x >= xL))):
            #{
                found = 1
                contin = 0
                R = self;
    ...
    #{
        ...
        #{
            ...
            #{
                contin = (SE_N < self.QElements);
                if SE_N < self.QElements:
                    contin=1
                else:
                    contin=0
             #}
            if(vsh!=0):
                print( "SE_N=" + str(SE_N) + " Q=" + str(self.QElements) + " contin=" + str(contin))
        #}
        if (found==1):
        #{
            #R = cur;#!
