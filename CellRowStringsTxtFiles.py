...
count=0
lines=[]
with open(r'c:\temp\pt.txt') as myFileWholeText:
    for line in myFileWholeText:
        count+=1
        print ("line: ",count,") ",line)
        print line
        #lines[count-1]=line#error
        lines.append(line)
        #L=line.count()
        #L=line.length()
        #L=line.len()
        ...
        n=line.count(substr)
        if n>0:
            N=line.index(substr) # error if not present
            print(substr," is present in line ",n," times, first is N ",N)
        else:
            print(substr," is in line ",n," times, simply is absent")
myFileWholeText.close()#no need: garbage colector does it gut
#
print"lines once more"
...
with open(r'c:\temp\new_pt.txt','w') as myfile:
    for element in lines:
        myfile.write(element)
    myfile.write('\n')
    myfile.write("This is sentence #1. ")
    myfile.write("This is sentence #2. ")
    myfile.write('\n')
    myfile.write(r'\n')
    myfile.write("This is sentence #3.\n ")
    myfile.write("This is sentence #4.\n ")
    myfile.close()
...
while True:
    line = file1.readline()
    if not line: # How does it work? Why ignores empty lines among existing?
        break
    print(line.strip())
file1.close
