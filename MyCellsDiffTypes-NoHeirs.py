...
class DataCell_ComboboxOrMemo:
    ...
    def GetItem(self, N=0):
        ...
        if isinstance(self.items, list):
            Q = MyLib.ArrayLength(self.items)
            if N >= 1 and N <= Q:
                R = self.items[N - 1]
            else:
...
class DataCellRow:
    ...
    def GetLength(self):
        return MyLib.ArrayLength(self.row)
    ...
    def SetContent(rows, LinesNotCols=true, LC_not_CL=true):
        if isinstance(rows, list):
            ...
            if LinesNotCols==LC_not_CL:
                ...
                QIneRows=(MyLib.DefExtrRowLensIn2DArr(rows))[2-1]
                for row in rows:
                    CurExtRow=[]
                    ColN=0    
                    for cell in row:
