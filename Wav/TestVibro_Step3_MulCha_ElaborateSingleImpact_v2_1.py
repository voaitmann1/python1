...
fileEnding="_SingleImpactRange.csv"
...
filenames =[]
#
# noam ecri ad file l'bounds, ma vals s'ecrid, ic s' in em, so S'not do tic file!
#
...
#
tHBnd=6#0#4#7#if tHBnd==7 so len(en1)=67200, len(env1)=66800   # let be 6 ob ce ablb idq uz all sensors
...
with open(filePath2+"\\"+"Results.csv", mode='a', newline='') as f:
    writer = csv.writer(f)
    writer.writerow([fileOwnNames[1-1], fileOwnNames[2-1], str(abs(beta1)), str(abs(beta2)), str(abs(betaS))])

print("done")
#-----------------------------------------------------
do_ElaborateFreqs=False
