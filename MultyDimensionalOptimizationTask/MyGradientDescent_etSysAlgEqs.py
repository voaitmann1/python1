...
FN=1 #1-task ab metoda, 2 - uz alg eqs sys
...
def main():
    ...
    if FN==1:
        ...
        x1 = 3#1.5 #-3#1 #1.5 # uz sys alg eqs
        x2 = 4#3.5 #5#2#3.5 # uz sys alg eqs
        gamma=0.0002
        alfa=0
    elif FN==3:
        x1 = 0#1.5 #-3#1 #1.5 # uz sys alg eqs
        x2 = 0#3.5 #5#2#3.5 # uz sys alg eqs
        gamma=0.002
        alfa=0
    ...
    while contin==1:
        ...
        for i in range (1, n+1):
            xpcd=Xprev[i-1]
            gradcd=grad[i-1]
            xcd=Xprev[i-1]-gamma*grad[i-1]*Xprev[i-1]#+alfa*(Xprev[i-1]-Xprev1[i-1])
        ...
        while(len(SNIter)<8):
            SNIter="0"+SNIter
        ss="Iter "+SNIter+" gamma="+str(gamma)
        ss=ss+" X=("
        for j in range (1, n):
            ss=ss+str(Xnext[j-1])+", "
        ss=ss+str(Xnext[n-1])+") "
