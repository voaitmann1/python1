...
def main():
    ...
    while contin==1:
        ...
        if localMin_reached==1:
            ...
            Rx=0
            for i in range (1, n+1):
                #Rx=Rx+Xprev[i-1]*Xprev[i-1]
                Rx=Rx+(Xprev[i-1]-X0[i-1])*(Xprev[i-1]-X0[i-1])
                #Rx=Rx+X0[i-1]*X0[i-1]
            Rx=Rx
            #for dirN in range(1, n+1+1):
            for dirN in range(0, n+1):
                Xdir=copy.deepcopy(Xprev)
                if dirN>0:
                    Xdir[dirN-1]=Xdir[dirN-1]+gamma*Rx+alfa*(Xprev[dirN-1]-Xprev1[dirN-1])
                print("      _"+" dirN="+str(dirN)+" Xprev="+str(Xprev)+" Xdir="+str(Xdir))
            dirN=-1
            while dirN<MaxDirN:
                dirN+=1
                gamma=gamma_ini
                ...
                if dirN>0:
                    Xdir[dirN-1]=Xdir[dirN-1]+gamma*Rx+alfa*(Xprev[dirN-1]-Xprev1[dirN-1])
                print("      "+" dirN="+str(dirN)+" Xprev="+str(Xprev)+" Xdir="+str(Xdir))
                grad=Grad(Xdir)
                countSubIter=0
                continIter=1
                while continIter==1:
                    ...
                    for i in range (1, n+1):
                        #x_cd=Xprev[i-1]
                        #gradcd=grad[i-1]
                        xnew_cd=Xdir[i-1]-gamma*grad[i-1]+alfa*(Xdir[i-1]-Xprev1[i-1])
                        #print("it."+str(K_iter)+" dim "+str(i)+" xcd="+str(xcd)+" g="+str(gradcd)+" xp="+str(xpcd))
                        Xnext.append(xnew_cd)
                    Ynext=whole_function(Xnext)
                    countSubIter+=1                     
                    ...
                    if gamma>gamma_ini/eps1 or countSubIter>100:
                        continIter=0
                        #contin=0
                    print("      "+" dirN="+str(dirN)+" iter="+str(countSubIter)+" gamma="+str(gamma)+" Xdp="+str(Xdir)+" X="+str(Xnext)+" Yprev="+str(Yprev)+" Ynext="+str(Ynext))
                    print("      "+" dirN="+str(dirN)+" MaxDirN="+str(MaxDirN)+" contin="+str(contin))
                #end while
            #end while
            if Ynext>Yprev:
                Xnext=X_LocMin
                ...
                print("local minimum avoided: X="+str(Xnext)+" Xp="+str(Xprev)+" gamma="+str(gamma)+" Ynext="+str(Ynext)+" Yprev="+str(Yprev))
        ...
        #
        if K_iter>4:
            Xprev4=copy.deepcopy(Xprev3)#Xprev4=Xprev3
            Yprev4=copy.deepcopy(Yprev3)#Xprev4=Xprev3
        if K_iter>3:
            Xprev3=copy.deepcopy(Xprev2)#Xprev3=Xprev2
            Yprev3=copy.deepcopy(Yprev2)#Yprev3=Yprev2
        if K_iter>2:
            Xprev2=copy.deepcopy(Xprev1)#Xprev2=Xprev1
            Yprev2=copy.deepcopy(Yprev1)#Yprev2=Yprev1
        if K_iter>1:
            Xprev1=copy.deepcopy(Xprev)#Xprev1=Xprev
            Yprev1=copy.deepcopy(Yprev)#Yprev1=Yprev
        Xprev=copy.deepcopy(Xnext)#Xprev=Xnext
        Yprev=whole_function(Xprev)
