...
class TableView:
    #
    def __init__(self, mstr, gui, contentSize, headerSize, activeRows, widthArray, Align_Left1Top2Bottom3Right4):
        ...
        self.FooterSize=TableSize(0,0)
        self.widthArray=copy.deepcopy(widthArray)
        for i in range(0, QLines+1):
            self.widthArray.append(widthArray[i])
        tableViewMainFrame=gui.Frame(master=mstr)
        ...
        if QLineHeaders>0 and QColumnHeaders>0:
            ...
            self.contentFrame=gui.Frame(master=tableViewMainFrame)
            self.contentFrame.pack(side=gui.RIGHT)
        elif QLineHeaders==0 and QColumnHeaders>0:
            #self.cornerFrame=gui.Frame(master=tableViewMainFrame)
            #self.cornerFrame.pack(side=gui.LEFT)
            self.colHeaderFrame=gui.Frame(master=tableViewMainFrame)
    ...
    def ShowGrid(self):
        ...
        for ColN in range(1, self.QColumns+1):
            for LineN in range(1,self.QLines+1):
                ...
                #MyNameText="cellLoCHtL"+str(LineN)+"C"+str(ColN) # need for future
                #MyNameText="cellCoLHL"+str(LineN)+"C"+str(ColN)  # need for future
                MyText="B"+str(LineN)+str(ColN)+" Name="+MyNameText
                #MyNameText="SL"+str(LineN)+"C"+str(ColN)#so err: widget name starts with uppercase letter
                #if (LineN==self.ActiveLineN and ColN==self.ActiveColumnN)
                #
                if LineN==activeRows.L and ColN==activeRows.C:
                    #CurLineCell=gui.Label(master=f, text=MyText, relief=tk.SUNKEN, background="blue")
                    curCntCell=gui.Label(master=curCntColFrame, name=MyNameText, text=MyText, relief=tk.SUNKEN)
                else:
                    curCntCell=gui.Label(master=curCntColFrame, name=MyNameText, text=MyText, relief=tk.RIDGE)
                curCntCell.pack()
                #
                curCntCol.append(curCntCell)
                curCntCol.pack(side=gui.LEFT)
            #f.pack()#also works
            cntCols.append(curCntCol)
        self
    ...
    #
    def GetCurRows(self):
        r=TableSize(activeRows.L, self.ActiveColN)
        return r
    #
