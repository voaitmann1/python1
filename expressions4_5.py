...
def CalcExpr(DataParts, trStrGiven):
        ...
        if trStrGiven.dataPart.QOpenBrackets>0:
            ...
            if (((op12Order>op01Order and prePart.QOpenBrackets==0 and (N==L or op12Order>=op23Order)) or curPart.QShutBrackets>0)and  curPart.QOpenBrackets==0) or (N==L and Ngiven==1):#hin add avan: and curPart.QOpenBrackets==0
                ...
                if op12Order>op01Order:
                    print("Case 3.1.1 - Immediate calc - with continuing: priority of cur operator priority ("+str(op12Order)+" opN="+str(op12)+" "+operatorNToString(op12, what_Alg1Query2)+") IS higher than of prev ("+str(op01Order)+" opN="+str(op01)+" "+operatorNToString(op01, what_Alg1Query2)+")")
                    #
                    #contin=1
                    if N<L:
                        N+=1
                ...
                #
                if curPart.QShutBrackets>0:
                    print("Case 3.1.2 - Immediate calc - with stopping recursion: bracket shut in cur expr element")
                    #
                    contin=0
                    if N<L:
                        N+=1
                ...
                #
                if N==L and Ngiven==1:
                    print("Case 3.2 - Immediate calc - with stopping recursion: : last expr reached - and it is not some inner recursion")
                    #
                    contin=0
    ...
    #while
    print("CalcExpr finishes working. Ngiven="+str(Ngiven)+" N="+str(N)+" yR="+str(rStr.dataPart.getVal())+"(full expr "+trStr.dataPart.ToString()+")")
    return rStr
#
