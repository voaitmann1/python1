...
def CalcQuery_ConsiderOrder_NoBrackets(QueryParts, Ngiven):
    L=len(QueryParts)
    N=Ngiven
    y1=QueryParts[N-1].calc()
    y2=0
    y3=0
    op12=QueryParts[N-1].Af_Or1And2EqSgnCalc3
    op23=0
    contin=1
    print("CalcQuery_ConsiderOrder_NoBrackets starts working. N=",N)
    print("Expr y1: "+QueryParts[N-1].ToString()+" = "+str(QueryParts[N-1].calc()))
    #print("Expr y1: "+QueryParts[N-1].ToString()," = ", str(y1))
    #print("Expr y1: "+QueryParts[N-1].ToString()," = ", y1)
    while contin==1:
        print("N="+str(N))
        if N==L or N>L or op12==3 or QueryParts[N-1].QShutBrackets>0:
            print("Solving immediately")
            y3=y1
            contin=0
        else:
            print("Analyzing")
            #print("Next: "+QueryParts[N+1-1].ToString()," = ", str(QueryParts[N+1-1].calc()))
            #print("Next: "+QueryParts[N+1-1].ToString()," = ", str(y2))
            #print("Next: "+QueryParts[N+1-1].ToString()," = ", y2)
            y2=QueryParts[N+1-1].calc()
            print("next Expr y2: "+QueryParts[N+1-1].ToString()+" = "+ str(QueryParts[N+1-1].calc()))
            #print("Expr y2: "+QueryParts[N+1-1].ToString()," = ", str(y2))
            #print("Expr y2: "+QueryParts[N+1-1].ToString()," = ", y2)
            op23=QueryParts[N+1-1].Af_Or1And2EqSgnCalc3
            if op12>=op23:
                print("priority of next "+QueryParts[N+1-1].AfToString()+" is NOT higher than of previous (cur) "+QueryParts[N-1].AfToString())
                if op12==1:
                    y3=y1+y2
                    #if y3>1:
                    #    y3=1
                ...
                if N+1<=L:
                    ...
                    y1=y3
                    op12=op23
                    #print("Now  y1= "+QueryParts[N-2-1].ToString()+" = "+str(QueryParts[N-2-1].calc()))
                    print("Now  y1 = "+str(y1)+" op12 = "+str(QueryParts[N+1-2-1].AfToString())+"="+AfToString(op12))
                else:
                    print("_last expr reached")
                    contin=0
            else:
                print("priority of next "+QueryParts[N+1-1].AfToString()+" IS higher than of previous (cur) "+QueryParts[N-1].AfToString())
                if N+1<=L:
                    N=N+1
                    y2, N = CalcQuery_ConsiderOrder_NoBrackets(QueryParts, N)
                    if op12==1:
                        #y3=y1+CalcQuery_ConsiderOrder_NoBrackets(QueryParts, N)
                        y3=y1+y2
                        #if y3>1:
                        #    y3=1
                    elif op12==2:
                        #y3=y1*CalcQuery_ConsiderOrder_NoBrackets(QueryParts, N)
                        y3=y1*y2
                    print("returninhg(1) to fn N",Ngiven,"y3=",y3)
                else:
                    print("last expr reached_")
                    contin=0
                #
            #
            #print("last cycle part")
            #if N+2<=L:
            #    print("continuing ")
            #    N=N+2
            #    y1=y3
            #    op12=op23
            #    print("y1=y3="+str(y1)+" N="+str(N)+" y2 startintg next")
            #    
            #    y2=CalcQuery_ConsiderOrder_NoBrackets(QueryParts, N)
            #else:
            #    contin=0
            #    print("last expression - stopping cycle")
            print("returninhg(2) to fn N",Ngiven)
        #
    #
    print("CalcQuery_ConsiderOrder_NoBrackets finishes working. N-=",Ngiven," y3=",y3)
    #return y3
    return y3, N
...
def Calc_ConsiderOrder_NoBrackets(DataParts, Ngiven):
    L=len(DataParts)
    what_Alg1Query2=0
    dataPart=0
    if(L>0):
    ...
    op23=0
    contin=1
    print("Calc_ConsiderOrder_NoBrackets starts working. N=",N)
    if(what_Alg1Query2==2):
    ...
    while contin==1:
        ...
        if N==L or N>L or op12==LastOrder or DataParts[N-1].QShutBrackets>0:
            ...
            if op12Order>=op23Order:
                ...
                if N+1<=L:
                    N=N+1
                    y2, N = Calc_ConsiderOrder_NoBrackets(DataParts, N)
                    if(what_Alg1Query2==2):
                        if op12==1:
                            y3=y1+y2
                            #if y3>1:
        ...
        #
    #
    print("Calc_ConsiderOrder_NoBrackets finishes working. N-=",Ngiven," y3=",y3)
    #return y3
    return y3, N
...
y, N=Calc_ConsiderOrder_NoBrackets(data, 1)#work't ety tak, ac y,n=... # n'int isinstance
