...
class DataCell_ComboboxOrMemo:
    ...
    def GetItem(self, N=0, vsh=0): #4
        R=0
        Q=len(self.items)
        if vsh==1:
            print("GetItem starts working")
            print("Q="+str(Q)+" N given="+str(N))
        if N==0:
            N=self.data
            if vsh==1:
                print("Now  N given="+str(N))
        if(N>=1 and N<=Q):
            R=self.items[N-1]
            if vsh==1:
                print("R="+str(R))
                print("GetItem finishes working")
        return R
...
class DataCell_DBTableHeader:
    ...
    def GetItems(self):
        R=[]
        R.append(self.GetVal())
        #R.append(self.data)
        #if self.DBTableData!=[]:
        #    R.append(self.DBTableData.TableNameInDB)
        #    if self.self.DBTableData.DBTableDataSuppl!=[]:
        #        R.append(self.DBTableData.DBTableDataSuppl.DBNameInDBCS)
        #        R.append(self.DBTableData.DBTableDataSuppl.DBTypeName)
        #        R.append(str(self.DBTableData.DBTableDataSuppl.DBTypeN))
        #        R.append(self.DBTableData.DBTableDataSuppl.DBFileFullName)
        return R
...
class DataCell:
    ...
    def SetCell_ColHeaderDBOrItems(self, val1="", val2=[] , val3=[], val4=[]):
    ...
    def Set(self, val1, val2=[], val3=[], val4=[]): #1
        if isinstance(val1, DataCell):
            ...
            self.cell=copy.deepcopy(val1)
        elif isinstance(val1, DataCell_DBTableHeader):
            self.cell=copy.deepcopy(val1)
        elif isinstance(val1, DataCell_ColHeaderDBOrItems):
            self.cell=copy.deepcopy(val1)
        elif not isinstance(val1, list) and val2==[] and val3==[] and val4==[]:
            self.cell=DataCell_Simple(val1)
        elif isinstance(val1, list) and isinstance(val2, int):
            self.cell=DataCell_ComboboxOrMemo(val1)
            self.cell.SetActiveN(val2)
        elif isinstance(val2, list) and isinstance(val1, int):
            self.cell=DataCell_ComboboxOrMemo(val2)
            self.cell.SetActiveN(val1)
        elif isinstance(val1, str) and isinstance(val2, list) and val3==[] and val4==[]:
            self.cell=DataCell_ComboboxOrMemo(val1)
            self.cell.SetActiveN(val2)
        #elif isinstance(val1, str) and isinstance(val2, list):
        #    self.cell=DataCell_ColHeaderDBOrItems(val1)
        #    self.cell.Set(val1, val2, val3, val4)
        elif isinstance(val1, str) and isinstance(val2, TDBTableData):
            self.cell=DataCell_DBTableHeader(val1)
            self.cell.Set(val1, val2, val3, val4)
        elif isinstance(val1, str) and (isinstance(val2, TDBFieldInfo) or isinstance(val3, TDBFieldInfo)):
            self.cell=DataCell_DBTableHeader(val1)
            self.cell.Set(val1, val2, val3, val4)
        elif isinstance(val1, str) and isinstance(val2, str) and isinstance(val3, TDBTableDataSuppl):
            self.cell=DataCell_DBTableHeader(val1, val2, val3)
...
class DataCellRow:
    ...
    def Set(self, row):#if row=[]=>row becomes []
        self.row=[]
        if isinstance(row, DataCellRowWithHeader):
            self.row=row.row
        elif isinstance(row, DataCellRow):
            self.row=row.row
    ...
    def GetContent_AsList_OfCells(self):
        return self.row

    def GetContent_AsList_OfVals(self):
        R=[]
        L=self.GetLength()
        for i in range(1, L+1):
            val=self.row[i-1].GetVal()
            R.append(val)
        return R

    def GetContent_AsList_OfCurItems(self):
        R=[]
        L=self.GetLength()
        for i in range(1, L+1):
            val=self.row[i-1].GetItem()
            R.append(val)
        return R

    def GetLength(self):          #2
        return len(self.row)
    ...
    def SetCellN(self, cellN, val1, val2=[], val3=[], val4=[]): #1
        Q=len(self.row)
        if cellN>=1  and cellN <= Q:
    ...
    #def GetSize():GetLength
    #    return len(self.row)
    def Fill(self, val):
        L=self.GetLength()
        cell=DataCell(val)
        for i in range(1, L+1):
            self.row[i-1]=copy.deepcopy(cell)
    #
    ...
    def SetSize(self, LNew, DfltVal=""):
        #LOld=self.GetSize()
        LOld=self.GetLength()
        cellDflt=DataCell(DfltVal)
        if LOld<LNew:
            for i in range(LOld, LNew+1):
                self.Add(DfltVal)
                #self.row.Add(DfltVal)
        elif LNew<LOld:
            del self.row[LNew:LOld+1]
...
class DataCellRowWithHeader:
    ...
    def Set(self, row, header=""):#if row=[]=>row becomes []
        self.row=[]
        if isinstance(row, DataCellRowWithHeader):
            self.row=row.row
            self.header=copy.deepcopy(row.header)
        elif isinstance(row, DataCellRow):
            self.row=row.row
            self.header=DataCell(header)
        elif isinstance(row, list):
            if len(row)>0:
                #Q=len(row)
                if len(row)==2:
                    if isinstance(row[1-1], list) and not isinstance(row[2-1], list):
                        content=copy.deepcopy(row[1-1])
                        header=copy.deepcopy(row[2-1])
                    elif not isinstance (row[1-1], list) and  isinstance(row[2-1], list):
                        content=copy.deepcopy(row[2-1])
                        header=copy.deepcopy(row[1-1])
                    elif not isinstance (row[1-1], list) and  isinstance(row[2-1], list):
                        content=copy.deepcopy(row)
                else:
                    content=copy.deepcopy(row)
    ...
    def ToString(self, delim="; ", hdrDelim=": ", sBef="", sAft="", hBef="", hAft=""):
        r=""
        Q=len(self.row)
        ...
        #
        if self.header!=0 and isinstance(self.header, DataCell):
            r=hdrDelim+r
            r=self.header.ToString(hBef, hAft)+r
        return r

    def SplitListToHeaderFirstAndContentRest(self, data, ContentReHeaderBef1Aft2=2):
        R=[]
        content=[]
        header=""
        Q=len(data)
    ...
    def Set_FirstElementAsHeader(self, row):
        data=self.SplitListToHeaderFirstAndContentRest(row, 2)
        self.Set(data)
    ...
    def GetContentWithHeader_AsListNested_OfCells(self):
        R=[]
        H=DataCell(self.header)
        R.append(H)
        R.append(self.row)
        return R

    def GetContentWithHeader_AsListNested_OfVals(self):
        C=[]
        R=[]
        H=self.header
        L=self.GetLength()
        ...
        if isinstance(self.header, DataCell):
            H=self.header.GetVal()
        R.append(H)
        R.append(C)
        return R

    def GetContentWithHeader_AsListNested_OfCurItems(self):
        C=[]
        R=[]
        H=self.header
        L=self.GetLength()
    ...
    def SetCell(self, cellN, val1, val2=[], val3=[], val4=[]): #1
        Q=len(self.row)
        if cellN>=1  and cellN <= Q:
            self.row[cellN-1].Set(val1, val2, val3, val4)
    ...
    def GetCell_AsCopy(self, cellN):
        ...
        if cellN>=1 and cellN<=Q :
            cell=copy.deepcopy(self.row[cellN-1])
        return cell
    def GetCell(self, cellN=0, AsLink0Copy1=1):
        cell=[]
        L=self.GetLength()
        if cellN==0:
            cellN=L
    ...
    def Fill(self, val):
        ...
        for i in range(1, L+1):
            self.row[i-1]=copy.deepcopy(cell)
    #
    def GetContentRow_AsList(self):
        row=DataCellRow(self.row)

    def GetContentRow_AsDataCellRow(self):
        return self.row
    #
    def ToStringN(self, cellN, sBef="", sAft=""):
        r=""
    ...
    def ToString_Content(self, delim="; ", sBef="", sAft=""):
        r=""
        Q=len(self.row)
        for i in range(1, Q-1+1):
            r=r+self.ToStringN(i, sBef, sAft)
    ...
    def SetSize(self, LNew, DfltVal=""):
        ...
        if LOld<LNew:
            for i in range(LOld, LNew+1):
                self.row.append(cellDflt)
        elif LNew<LOld:
            del self.row[LNew:LOld+1]

    # header --------------------------------------------------

    def GetType_header(self):          #2
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetType()
        return r

    def SetCell_Simple_header(self,  val1):
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.SetCell_Simple(val1)

    def SetCell_ComboboxOrMemo_header(self,  val1=[], val2=1):
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.SetCell_ComboboxOrMemo(val1, val2)

    def SetCell_DBTableHeader_header(self,  val1="", val2=[] , val3=[]):
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.SetCell_DBTableHeader(self, val1, val2 , val3)

    def DataCell_ColHeaderDBOrItem_headers(self,  val1="", val2=[] , val3=[], val4=[]):
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.SetCell_DBTableHeader(self, val1, val2 , val3, val4)

        
    def GetVal_header(self,  N=1): #3
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetVal(N)
        return r

    def GetItems_header(self):
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=header.GetItems()
        return r

    def GetItem_header(self,  N=0): #4
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetItem(N)
        return r
        
     # ConcrType
    def GetFloatVal_header(self): #5
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetFloatVal()
        return r

    def GetIntVal_header(self): #6
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=header.GetIntVal()
        return r

    def GetBoolVal_header(self): #7
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetBoolVal()
        return r

    def GetStrVal_header(self): #8
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetStrVal()
        return r

    # for combobox
    def GetActive_headerN(self): #for combobox-1
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetActiveN()
        return r

    def GetActiveItem_header(self): #for combobox-2
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetActiveItem()
        return r
    
    def SetActive_headerN(self, val): #for combobox-3
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetActiveN(val)
        return r
    #
    def SetActiveNByVal_header(self, val): #for combobox-4
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.SetActiveNByVal(val)
        return r
        #
    def GetQItems_header(self): #for combobox-5
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetQItems()
        return r

    def SetCell_header(self,  val1, val2=[], val3=[], val4=[]): #1
       if self.header!=0 and isinstance(self.header, DataCell):
            self.header.Set(val1, val2, val3, val4)

    def SetVal_header(self,  val, N=0): #1
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetVal(val, N)
        
    def GetTableNameToDisplay_header(self):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetTableNameToDisplay()
        return r

    def GetTableNameInDB_header(self):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetTableNameInDB()
        return r

    def GetDBNameInDBCS_header(self):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetDBNameInDBCS()
        return r

    def GetDBTypeName_header(self):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetDBTypeName()
        return r
    
    def GetDBTypeN_header(self):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetDBTypeN()
        return r

    def GetDBFileFullNam_headere(self):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetDBFileFullName()
        return r

    #
    def SetTableNameToDisplay_header(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetTableNameToDisplay(val)

    def SetTableNameInD_headerB(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetTableNameInDB(val)
        
    def SetDBNameInDBCS_header(self, val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetDBNameInDBCS(val)

    def SetDBTypeName_header(self,  val):
       if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetDBTypeName(val)

    def SetDBType_headerN(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetDBTypeN(val)

    def SetDBFileFullName_header(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetDBFileFullName(val)

    #
    def SetColNameToDispla_headery(self,  val):
       if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetColNameToDisplay(val)
    def SetDBFieldNam_headere(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetDBFieldName(val)
    def SetFieldType_headerN(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.row[cellN-1].SetFieldTypeN(val)
    def SetFieldTypeNam_headere(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetFieldTypeName(val)
    def SetFieldLengt_headerh(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetFieldLength(val)
    def SetDBFieldCharacteristicsNumbe_headerr(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetDBFieldCharacteristicsNumber(val)
    def SetIfIsKeyField_header(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetIfIsKeyField(val)
    def SetIfIsCounte_headerr(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetIfIsCounter(val)
    def SetIfIsNotNull_header(self,  val):
       if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetIfIsNotNull(val)
    def SetIfIsAutoIncrement_header(self,  val):   
       if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetIfIsAutoIncrement(val)
    #
    def SetItemsTableNam_headere(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetItemsTableName(val)
    def SetItemsTableItemsFieldName_header(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetItemsTableItemsFieldName(val)
    def SetItemsTableKeyFieldNam_headere(self,  val):
        if self.header!=0 and isinstance(self.header, DataCell):
            self.header.SetItemsTableKeyFieldName(val)
    #
    def GetColNameToDisplay_header(self):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetColNameToDisplay()
        return r
    def GetDBFieldNam_headere(self, cellN):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetDBFieldName()
        return r
    def GetFieldTypeN_header(self):
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetFieldTypeN()
        return r
    def GetFieldTypeName_header(self):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetFieldTypeName()
        return r
    def GetFieldLength_header(self):
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetFieldLength()
        return r
    def GetDBFieldCharacteristicsNumber_header(self):
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetDBFieldCharacteristicsNumber()
        return r
    def GetIfIsKeyField_header(self):
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetIfIsKeyField()
        return r
    def GetIfIsCounter_header(self):
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetIfIsCounter()
        return r
    def GetIfIsNotNull_header(self):
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetIfIsNotNull()
        return r
    def GetIfIsAutoIncrement_header(self):   
        r=0
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetIfIsAutoIncrement()
        return r
    #
    def GetItemsTableName_header(self):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetItemsTableName()
        return r
    def GetItemsTableItemsFieldName_header(self):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetItemsTableItemsFieldName()
        return r
    def GetItemsTableKeyFieldName_header(self):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=self.header.GetItemsTableKeyFieldName()
        return r
    #
    # Set
    #            
    ...
    #def Del(self, cellN):
    #    Del1DArrayElement(self.row, cellN)
    def GetCell_AsLink_header(self):
        cell=[]
        if self.header!=0 and isinstance(self.header, DataCell):
            cell=self.header
        return cell
    def GetCell_AsCopy_header(self):
        cell=[]
        if self.header!=0 and isinstance(self.header, DataCell):
            cell=copy.deepcopy(self.header)
        return cell
    def GetCell_header(self,  AsLink0Copy1=1):
        cell=[]
        if AsLink0Copy1==0:
            cell=self.GetCell_AsLink_header()
        else:
            cell=self.GetCell_AsCopy_header()
        return cell
    #def SetCell_header(self, val):
     #   if val=="":
    #        self.header=0
     #   else:
    #        self.header=DataCell(val)
    def SetCell_header(self, val1="", val2=[], val3=[], val4=[]):
        if self.header==0 and not isinstance(self.header, DataCell):
            self.header=DataCell()
        self.header.Set(val1, val2, val3, val4)
    #def Swap(self, cell1N, cell2N):
    #    Swap1DArrayElements(self.row, cell1N, cell2N)
    #def GetSize():
    ...
    def ToString_header(self, sBef="", sAft=""):
        r=""
        if self.header!=0 and isinstance(self.header, DataCell):
            r=r+self.header.ToString(sBef, sAft)
        return r
