...
def CalcExpr(DataParts, trStrGiven):
    ...
    while contin==1:
        ...
        if trStrGiven.dataPart.QOpenBrackets>0:
            #trStr is kept
            trStr=trStrGiven
            trStr.N=N#es, N=N given s'am start
            trStr.dataPart.QOpenBrackets-=1
            #trStr.setVal(0)
            trStr.dataPart.setOperatorN(0)
            print("Bracket open (in pre part; left: "+str(trStr.dataPart.QOpenBrackets)+"). Starting recursion (ob ()) with operand="+str(trStrGiven.dataPart.getVal())+", operator=0, N="+str(N))
            trStr=CalcExpr(DataParts, trStr)
            #
            N=trStr.N
            print("returned from recursion (ic ob ()). N="+str(trStr.N)+"=N="+str(N)+" to Ngiven="+str(Ngiven))
            contin=0
            #hin ak ce wa - no rStr struct s'def'd
            #so I add all ef elif
            rStr=trStr # rStr.operat=trStr.operat
            rStr.N=N+1 # n'inty qob, ma, seems, so calc > ver
            ...
            print ("Analyzing")
            print("Btackets of parts: prePart: open: "+str(prePart.QOpenBrackets)+" shut: "+str(prePart.QShutBrackets)+"; cuPart: open: "+str(curPart.QOpenBrackets)+" shut: "+str(curPart.QShutBrackets))
            ...
            if (op01Order>=op12Order and prePart.QOpenBrackets==0) or curPart.QShutBrackets>0 or N==L:
                ...
                if op01Order>=op12Order:
                    ...
                    print("(order of prev oper "+OperatorNToString(op01)+"(opN="+str(op01)+" ordN="+str(op01Order)+")"+" is NOT greater than of cur "+OperatorNToString(op12)+"(opN="+str(op12)+" ordN="+str(op12Order)+")"+")")
                #
                if N==L:
                    print("case 3.2 last_expr_part reached")
                    if Ngiven>1:
                        print("last expr reached, but not in ext fn - no calc provided")
                    #
                #
                #if (N==L and Ngiven==1):
                if (N==L and Ngiven==1) or (curPart.QShutBrackets>0 and (prePart.QOpenBrackets>0 or op01Order<=op12Order)):
                #if Ngiven==1:
                    print("case to calc")
                    if (N==L and Ngiven==1):
                        print("3.4 N==L && Ngiven=1")
                    #
                    if curPart.QShutBrackets>0 and prePart.QOpenBrackets>0:
                        print("3.3.2 expr with 1 operand in brackets")
                    #
                    if curPart.QShutBrackets>0 and op01Order<=op12Order:
                        print("3.3.1 shut bracket before operator of higher priority")
                    #
                    if what_Alg1Query2==2:
                        if op12==1:
                            y3=y1+y2
                ...
                if curPart.QShutBrackets>0:
                    ...
                    #
                    #if prePart.QOpenBrackets>0 or op01Order<=op12Order:
                    #    if prePart.QOpenBrackets>0:
                    #        print("single action in brackets")
                    #    #
                    #    if op12Order<=op12Order:
                    #        print("bracket is shut with most prior operator")
                    #    #
                    #    y3=Calc(y1, op12, y2)
                    #    y1=y3
                    ##
                ...
                if op12Order>=op23Order and curPart.QOpenBrackets==0:
                    ...
                    if curPart.QOpenBrackets>0:
                        ...
                        print("(cur part: _0_ brackets open)")
                    #
        ...
        #
    #while
    print("CalcExpr finishes working. Ngiven="+str(Ngiven)+" N="+str(N)+" N returned: "+str(rStr.N)+" yR="+str(rStr.dataPart.getVal())+"(full expr "+trStr.dataPart.ToString()+")")
    return rStr
#   
