...
def Gradient_and_Hesse(X, funclink, what_g0H1=1, delta=0.000001, vsh=0):
    if vsh==1:
...
def fGradient(X, funclink=0, vsh=0):
    global ChosenDerivCalMethodN_Analyt0Num1
    R=[]
    if ChosenDerivCalMethodN_Analyt0Num1==0:
    ...
    #Gradient_and_Hesse(X, funclink, what_g0H1=1, delta=0.000001, vsh=0)
        print("Calc g and H numerically")
        what_g0H1=0
        delta=0.000001
        R=Gradient_and_Hesse(X, funclink, what_g0H1, delta, vsh) 
    return R
...
def Hessian(X, funclink=0, vsh=0):
    global ChosenDerivCalMethodN_Analyt0Num1
    R=[]
    if ChosenDerivCalMethodN_Analyt0Num1==0:
        print("Calc g and H analytically")
...
def fXNew_Cauchee(alfa, X0, funclink=0, vsh=0):
    global ChosenDerivCalMethodN_Analyt0Num1
    X1=[]
    if ChosenDerivCalMethodN_Analyt0Num1==1:
        ...
        what_g0H1=0
        delta=0.000001
        #GnG= Gradient_and_Hesse(X0, funclink, what_g0H1, delta, vsh)
        g=fGradient(X0, funclink, vsh)
        L=len(X0)
        for i in range(1, L+1):
            cur=X0[i-1]-alfa*g[i-1]
            X1.append(cur)
    else:
        x1_0=X0[1-1]
        x2_0=X0[2-1]
...
def fXNewNewton(X, funclink, vsh=0):
    if vsh==1:
    ...
    s=[]
    order=len(X)
    g=fGradient(Xprev, funclink, vsh)
    neg=g*(-1)
    H=Hessian(Xprev, funclink)
...
def fXNewNewtonMod(alfa, X, funclink, vsh=0):
    if vsh==1:
        print("fXNewNewton srarts working")
    Xprev=copy.deepcopy(X)
    Xnext=copy.deepcopy(Xprev)
...
def fXNewFlatcher(alfa, X, funclink, d, vsh=0):
    Xprev=copy.deepcopy(X)
    if vsh==1:
...
def fXNewMarquardt(alfa, X, funclink, vsh=0):
    if vsh==1:
...
def MyOptimNewton(funclink, X_ini_guess, cfg, vsh=0):
    ...
    while contin==1:
        itersCount+=1
        Xnext=fXNewNewton(Xprev, funclink, vsh)
        #
        if abs(funclink(Xnext))<cfg.getPrecY() or fCompareX(Xprev, Xnext, cfg.getPrecX())==1 or itersCount>=cfg.getQIters():
            contin=0
        Xprev=Xnext
...
def MyOptimMarquardt(funclink, X_ini_guess, cfg, vsh=0):
    ...
    while continIterationsSuccession==1:
        ...
        #
        cond_gradient=(abs(fVectModule(fGradient(Xnext, funclink)))<cfg.getPrecY())
...
def main():
    ...
    if ChosenMethodN==Cauchee_MethodN:
        ...
        print( fXNewFlatcher(0.5, X_ini_guess, functionToTest, 1))
    #
