...
class DataCell_ColHeader_DBFieldOrItems:
            ...
            if isinstance(DBFldInfo, TDBFieldInfo):
                ...
                if len(DBFldInfo)>0:
                    ...
                    if len(self.DBFieldInfo)>=8:
                        ...
                        if self.DBFieldInfo.DBFieldInfoSuppl=TDBFieldInfoSuppl()
    ...
    def SetIfIsKeyField(self, isKeyField): b #ColDBHeaderItems-21
        if self.DBFieldInfo==[]:
            self.DBFieldInfo=TDBFieldInfo()
            if self.DBFieldInfo.DBFieldInfoSuppl==[]:
                self.DBFieldInfo.DBFieldInfoSuppl=TDBFieldInfoSuppl()
...
class Table1:
    ...
    def SetCell_LineOfColHeader(self, val, N, ExistingVal_Allowed0Forbidden1=0):
        #if(not(isinatance(self.LineOfColHeader, My1DArray))):
        #    self.LineOfColHeader=My1DArray()
        if isinstance(self.LineOfColHeader, My2DArray):
            self.LineOfColHeader.Ins(N, val, 
...
class DataCellRow1:
   ...
   def SeekFirst(self, val, FromN=1)#copied et mod'd, ob "=" for cells is special #->My1DArray-#11
    ...
    #Functions of DataCell
    #
    def Set_Simple(self, N, var1, ExistingVal_Allowed0Forbidden1=0, ExistingVal_Allowed0Forbidden1=0): #1-11-1
        Q=self.GetLength()
        if N>=1 and N<=Q:
...
class DataCellRowWithHeader1:
   ...
   #         #can't row[ColN-1] 
    ...
    #
    def Set_Simple_Hdr(self, var1, ExistingVal_Allowed0Forbidden1=0, ExistingVal_Allowed0Forbidden1=0): #1-11-1
    ...
    def ToString_Info(self, delim=" ", str_bef="", str_aft=""):
        ...
        if Q>0:
            s=s+ToString_Info_Element(self, Q, str_bef, str_aft
        s=s+" Header: "
        s=s+ToString_Info_RowHdr(self, Q, str_bef, str_aft
        return s
