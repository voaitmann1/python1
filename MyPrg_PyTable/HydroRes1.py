...
class HResSimple:
    def __init__(self, k=0, Conn_Suc0Par1=0, Ress=[], ownN=0, outerN=0, upper=0):
    ...
    def Calc(self, vsh=0):
        ...
        if self.Conn_Suc0Par1==0:
            ...
            if(vsh==1):
                print('parallel conn')
            for i in range(1, L+1):
                C=1/self.Elements[i-1].Calc(vsh)
                if(vsh==1):
                    print("prev Calc finished, external one continueing")
                S=S+C
                if(vsh==1):
    ...
    def Add(self, ElementExt, vsh=0):
        Element=copy.deepcopy(ElementExt)
        Element.outerN=len(self.Elements)
        Element.upper=self
        self.Elements.append(Element)
        if(vsh==1):
            print(self.ToString())
            print("Added:")
            print(self.Elements[len(self.Elements)-1].ToString())
    ...
    def Del(self, N):
        ...
        if N>=1 and N<=len(self.Elements):
            for i in range(1, N-1+1):
                Element=copy.deepcopy(self.Elements[i-1])
                row.append(Element)
    ...
    def InsN(self, ElementExt, N):
        ...
        if N>=1 and N<=Q:
            ...
            if(vsh==1):
                print(self.ToString())
                print("Ins N"+str(N)+":")
                print(self.Elements[N-1].ToString())

    def vis_L(self, vsh=0):
        if vsh==1:
            print("vis_L finishes working (R"+str(self.ownN)+"L="+str(L)+")")
        L=1#own or left wall
        if self.upper==0:
            L=L+1#left connector
            if vsh==1:
                print("ext: Left connector + Left wall: L="+str(L))
        if isinstance(self.Elements, list):
            if vsh==1:
                if self.Conn_Suc0Par1==0:
                    for i in range(1, len(self.Elements)):
                        ...
                        L=L+Lcur
                        L=L+1
                    Lcur=self.Elements[len(self.Elements)-1].vis_L(vsh)
                    L=L+Lcur
                else:
                    L=L+1
                    Lmax=0
            ...
            if vsh==1:
                if self.upper==0:
                    print("L=1="+str(L)+" (own length)")
                print("nil to add")
        if self.upper==0:
            L=L+2#Right connector + Right wall
            if vsh==1:
                print("ext: Right connector + Right wall: L="+str(L))        
        if vsh==1:
            print("vis_L finishes working (R"+str(self.ownN)+"L="+str(L)+")")
        return L
    ...
    def ToString(self):
        s=""
        s=s+"R"+str(self.ownN)+" k="+str(self.k)+" Sub-Elts: "
        if self.Conn_Suc0Par1==0:
            s=s+"--"
        else:
            s=s+"||"
        ...
        if(self.upper==0):
            ...
            s=s+" (Of: R"+str(self.upper.ownN)+" ["+str(self.outerN)+"])"
        return s
