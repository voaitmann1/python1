...
def FillArray1D(arr, val, l=0):#new
    Q=len(arr)
    if Q>0:
        del(arr[:])
        if l==0:
...
def SeekFromStart(Where, What, FromN=1, ToN=0, vsh=0):
    if vsh==1:
...
def SeekFromEnd(Where, What, FromN, ToN=0, vsh=0):
    ...
    Ns = SeekFromStart(Where, wht, ToN, FromN, vsh)
    count=len(Ns)
    if vsh==1:
        print("Found="+str(count)+":")
        print(Ns)
...
def SeekFirst(Where, What, FromN=1, ToN=0, vsh=0):
    Ns=SeekFromStart(Where, What, FromN, ToN, vsh)
    if Ns==[]:
        y=0
    else:
        y=Ns[1-1]
...
def Set1DArrayLength(arr, Lnew, DfltVal="0"):
    Lold=len(arr)
    if Lold>Lnew:
        del arr[Lnew:Lold+1]
...
def FillArray1D(arr, val):#ne tested
    Q=len(arr)
    del arr[:]
    for i in range(1, Q+1):
        arr.append(val)

def NewValUniqueFor1DArray(arr, val, N, sBef="", sAft="", vsh=0):#ne tested
    if vsh==1:
        print("NewValUniqueFor1DArray starts working")
    R=copy.deepcopy(val)
    if vsh==1:
        print("val given:")
        print(str(val))
    if arr==[]:
        if vsh==1:
            print("empty arr given!")
    else:
        contin=1
        count=0
        if vsh==1:
            print("starting correction cycle for unique val")
        while contin==1:
            NExisting=SeekFirst(arr, val)
            if vsh==1:
                print("step "+str(count)+": found at N "+str(NExisting))
                print (val)
            if NExisting!=0:
               val=str(val)+sBef+str(N)+sAft
               if vsh==1:
                   print("value already exists - correcting - now having:")
                   print (val)
            else:
                contin=0
                if vsh==1:
    ...
    if vsh==1:
        print("final val:")
        print(val)
        print("NewValUniqueFor1DArray finishes working")
    return R
...
def Add2DArrayIneRow(arr, rowExt=[], DfltVal=0, NonRectAllowed=0, vsh=0):
    if vsh==1:
        print("Ins2DArrayExtRow starts working")
    ...
    LP=len(rowToAdd)
    if vsh==1:
        print("Before add: LE="+str(LE)+" Lmin="+str(Lmin)+" Lmax="+str(Lmax)+" LP="+str(LP))
    if NonRectAllowed==0:
        if Lmin<Lmax: 
            Stretch2DArray(arr, Lmax, DfltVal)
            if vsh==1:
                print("NonRectAllowed==0 Lmin<Lmax")
        if LP<LE:
            for i in range(LP+1, LE+1):
                rowToAdd.append(DfltVal)
            if vsh==1:
                print("NonRectAllowed==0 LP<LE")
        elif LP>LE:
            del(rowToAdd[LE:LP])
            if vsh==1:
                print("NonRectAllowed==0 LP>LE")
    LP1=len(rowToAdd)
    if vsh==1:
        print("Now LP1="+str(LP1))
    for i in range(1, LP1+1):
        if vsh==1:
            print("element "+str(i)+"before add: L="+str(len(arr[i-1])))
    ...
    Lmax=Get2DArrayMaxLength(arr)
    if vsh==1:
        print("After add: LE="+str(LE)+" Lmin="+str(Lmin)+" Lmax="+str(Lmax))
...
def SetRect2DArray(arr, L=0, QE=0, dfltVal=0, vsh=0):
    row=[]
    maxL=Get2DArrayMaxLength(arr)
    minL=Get2DArrayMaxLength(arr)
    if L==0:
        L=maxL
    Q=len(arr)
    if QE==0:
        QE=Q
    if vsh==1:
        print("SetRect2DArray starts working")
        print("array given:")
        ShowToConsole2DArrayWhole(arr, " ")
        print("Transforming to "+str(QE)+" x "+str(L)+":")
    for i in range(1, Q+1):
        curL=len(arr[i-1])
        if L>curL:
            for j in range(curL+1, L+1):
                arr[i-1].append(dfltVal)
            if vsh==1:
                newL=len(arr[i-1])
                print(str(i)+") - stretched, new L="+str(newL)+":")
                s=""
            ...
            if vsh==1:
                newL=len(arr[i-1])
                print(str(i)+") - trimmed, new L="+str(newL)+":")
                s=""
                for j in range(1, L+1):
                    s=s+str(arr[i-1][j-1])+" "
                print(s)
            ...
            if vsh==1:
                print("not changed, same length")
    if Q>QE:
        del(arr[Q-1:])
        if vsh==1:
            print("superfluous lines deleted")
    elif Q<QE:
        #row=[]
        #for i in range(1, L+1):
        #    row.append(dfltVal)
        #for i in range(Q+1, QE+1):
        #    arr.append(row)
        for i in range(Q+1, QE+1):
            row=[]
            for j in range(1, L+1):
            ...
            arr.append(row)
        if vsh==1:
            print("empty lines added")
    if vsh==1:
        print("now array is:")
        ShowToConsole2DArrayWhole(arr, " ")
        print("SetRect2DArray finishes working")
                        

def Array2DConcatAddingVertically(arrToAppend, arrToAdd):
    for i in range(1,len(arrToAdd)+1):
        arrToAppend.append(arrToAdd[i-1])

def Array2DConcatStretchingHorisontally(arrToStretch, arrToAdd, QShift=0, How_LastM2_MaxM1_AnyOther=-2, dfltVal=0, vsh=1):
    #if(QShift==0 and len(arrToStretch)==len(arrToAdd) and Get2DArrayMinLength(arrToAdd)==Get2DArrayMaxLength(arrToStretch) and
    QToAdd=len(arrToAdd)
    QToStretch=len(arrToStretch)
    NToStretch=0
    LToAddTo=0
    if How_LastM2_MaxM1_AnyOther==-2:
        for i in range(1, QToAdd+1):
            NToStretch=i+QShift
            if NToStretch<=QToStretch:
                LCurToStretch=len(arrToStretch[NToStretch-1])
                LCurToAdd=len(arrToAdd[i-1])
                for j in range(1, LCurToAdd+1):
                    arrToStretch[NToStretch-1].append(arrToAdd[i-1][j-1])
            else:
                arrToStretch.append(arrToAdd[i-1])
                
    else:
        if How_LastM2_MaxM1_AnyOther>-2:
            if How_LastM2_MaxM1_AnyOther==-1:
                LToAddTo=Get2DArrayMaxLength(arrToStretch)
            else:
                LToAddTo=How_LastM2_MaxM1_AnyOther
            if QToAdd+QShift>QToStretch:
                QNew=QToAdd+QShift
            else:
                QNew=QToStretch
            SetRect2DArray(arrToStretch, LToAddTo, QNew, dfltVal, vsh)
            for i in range(1, QToAdd+1):
                NToStretch=i+QShift
                LCurToAdd=len(arrToAdd[i-1])
                LWawsToStretch=len(arrToStretch[NToStretch-1])
                print("Line "+str(NToStretch)+" was")
                s=""
                for j in range(1, LWawsToStretch+1):
                    s=s+str(arrToStretch[NToStretch-1][j-1])+" "
                print(s)
                for j in range(1, LCurToAdd+1):
                    arrToStretch[NToStretch-1].append(arrToAdd[i-1][j-1])
                    if vsh==1:
                        print("Line "+str(NToStretch)+" adding X["+str(i)+", "+str(j)+"]="+str(arrToAdd[i-1][j-1]))
                print("Line "+str(NToStretch)+" is")
                s=""
                for j in range(1, LWawsToStretch+LCurToAdd+1):
                    s=s+str(arrToStretch[NToStretch-1][j-1])+" "
                print(s)
